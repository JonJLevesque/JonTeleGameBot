...
def backup(dest_dir: str, keep: int = 14) -> str:
    ...
    dest = os.path.join(
        dest_dir, f"ajbot-{time.strftime('%Y%m%d-%H%M%S')}.db"
    )
    with sqlite3.connect(dest) as target:
        _db().backup(target)
    for old in sorted(glob.glob(os.path.join(dest_dir, "ajbot-*.db")))[:-keep]:
        os.remove(old)
    return dest


def latest_backup_age_hours(dest_dir: str) -> float | None:
    files = glob.glob(os.path.join(dest_dir, "ajbot-*.db"))
    if not files:
        return None
