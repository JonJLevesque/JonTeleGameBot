...
def test_ledger_round_trip_and_settle(db):
    i = db.iou_add(1, 2, "Audrey", 1, "J", "a back rub", "manual", T0)
...
def test_cancel_only_open(db):
    i = db.iou_add(1, 2, "Audrey", 1, "J", "dinner", "manual", T0)
...
def test_ledger_text_groups_by_debtor(db):
    db.iou_add(1, 2, "Audrey", 1, "J", "a coffee", "manual", T0)
    db.iou_add(1, 2, "Audrey", 1, "J", "dinner", "shop", T0 - 20 * 86400)
    db.iou_add(1, 1, "J", 2, "Audrey", "a song", "manual", T0)
...
def test_shop_add_records_owner_and_debtor_rules(db):
    item_id = db.shop_add(1, 50, "loser cooks dinner", 7, "Audrey")
    item = db.shop_get(1, item_id)
    assert (item["owner_id"], item["owner_name"]) == (7, "Audrey")
    assert debtor_for(item, buyer_id=1, chat_id=1, duo=False) == (7, "Audrey")
    ...
    db.remember_user(1, FakeUser(7, "Audrey"))
...
def test_recap_mentions_ious(db):
    ...
    db.iou_add(1, 2, "Audrey", 1, "J", "a coffee", "manual", T0)
    text = recap._build(1)
    assert "IOUs: 1 open" in text and "Audrey owes a coffee" in text
